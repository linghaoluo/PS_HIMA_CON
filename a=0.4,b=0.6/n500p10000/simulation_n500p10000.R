...
results_Pscw_n500p10000<-list() 
results_Iptw_n500p10000<-list() 
results_Pscov_n500p10000<-list() 
results_Non_n500p10000<-list() 
results_Cov_n500p10000<-list() 
...
for (v in 1:500) {
  ...
  alpha[1:8] <- c(0.20,0.24,0.30,0.32,0.40,0.48,0.60,0.80)
  beta1[1:8] <- c(0.30,0.36,0.45,0.48,0.60,0.72,0.90,1.20)
  #beta2[1:4] <- c(1.45,1.5,1.55,1.6)
  # these are not true mediators 
  alpha[11:12] <- 0.60
  beta1[9:10] <- 0.90
  sim_data<-simdata(n,p,alpha,beta1,seed=v)
  results_Pscw_n500p10000[[v]]=myhima(sim_data,method='Psuni',v)
  results_Iptw_n500p10000[[v]]=myhima(sim_data,method='Iptw',v)
  results_Pscov_n500p10000[[v]]=myhima(sim_data,method='Pscov',v)
  results_Non_n500p10000[[v]]=myhima(sim_data,method='Non',v)
  results_Cov_n500p10000[[v]]=myhima(sim_data,method='Cov',v)
}
